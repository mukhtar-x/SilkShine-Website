                    ...
                    <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-3xl p-6 md:p-10 border border-amber-500/10 shadow-xl flex flex-col justify-between">
                        ...
                        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto w-full items-start">
                            <div className="bg-gray-50/80 dark:bg-gray-800/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
                                <h2 className="text-lg font-bold mb-4 dark:text-white">{translate('contactUs')}</h2>
                                {submitStatus === 'success' && (
                                    <div className="rounded-xl border border-green-200 bg-green-50 text-green-700 px-4 py-3 mb-4 text-sm">
                                        {translate('formSubmitSuccess')}
                                    </div>
                                )}
                                {submitStatus === 'error' && (
                                    <div className="rounded-xl border border-red-200 bg-red-50 text-red-700 px-4 py-3 mb-4 text-sm">
                                        {submitError || translate('formSubmitError')}
                                    </div>
                                )}
                                <form className="space-y-3" onSubmit={handleSubmit}>
                                    <div className="grid md:grid-cols-2 gap-3">
                                        <input
                                            ...
                                            className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-xs outline-none focus:ring-2 ring-amber-500 dark:text-white w-full"
                                        />
                                        <input
                                    ...
                                    <textarea
                                        ...
                                        className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-xs outline-none focus:ring-2 ring-amber-500 dark:text-white w-full resize-none"
                                    />
                                    <button
                                        type="submit"
                                        disabled={isSubmitting}
                                        className="w-full bg-amber-500 hover:bg-amber-600 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 text-xs shadow-md shadow-amber-500/20"
                                    >
                                        <Send className="w-4 h-4" /> {translate('sendMessage')}
                                    </button>
                                </form>
                            </div>
                                ...
                                {CONTACT_DATA.info.map((item, idx) => (
                                    <div key={idx} className="flex items-center gap-4 p-4 bg-gray-50/80 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{t(item.value)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
