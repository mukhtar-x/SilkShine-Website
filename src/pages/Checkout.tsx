                ...
                {step === 'success' ? (
                    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4 animate-fade-in-up">
                        ...
                        <p className="text-gray-500 mb-8 max-w-md">
                            Thank you for choosing SilkShine. Your order has been confirmed and will be shipped to {formData.address}[cite: 13].
                        </p>
                        <Link to="/" className="bg-slate-950 dark:bg-white text-white dark:text-slate-950 px-8 py-3 rounded-full hover:opacity-90 transition-colors font-bold">
                            {t('browseProducts')}
                        </Link>
                    </div>
                ) : cart.length === 0 ? (
                    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
                        ...
                        <Link to="/products" className="text-amber-600 hover:text-amber-700 font-bold underline">
                            {t('browseProducts')}
                        </Link>
                    </div>
                ) : (
                                ...
                                <div className="bg-white border-2 border-gray-100 shadow-lg dark:bg-gray-800 p-6 rounded-3xl sticky top-24 dark:border-gray-700">
                                        ...
                                        <div className="space-y-3 mb-6 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                            {cart.map((item) => (
                                                <div key={item.id} className="flex gap-3 items-center">
                                                    ...
                                                    <div className="text-sm font-bold dark:text-white">Rs. {(item.price * item.quantity).toLocaleString()}[cite: 13]</div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {step === 'details' && (
                                        <button
                                            form="checkout-form"
                                            type="submit"
                                            className="w-full bg-slate-950 dark:bg-white text-white dark:text-slate-950 font-bold py-4 rounded-xl mt-6 hover:opacity-90 transition-opacity shadow-lg"
                                        >
                                            {t('placeOrder')} (Rs. {finalTotal.toLocaleString()})
                                        </button>
                                    )}
                                </div>
                            </div>
