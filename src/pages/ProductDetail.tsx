...
const ProductDetail: React.FC = () => {
    ...
    // Helper to translate object {en, ur}
    const tp = (obj: any) => obj[language] || obj['en'];

    // Fix: Convert string ID to number for lookup
    const product = PRODUCTS.find((p) => p.id === Number(id));

    // State for gallery and size
    ...
    // Use local assets for the gallery to fill space
    const images = [
        product.image,
        about1,
        about2
    ...
    return (
        <div className="container mx-auto px-4 py-8 animate-fade-in-up max-w-6xl">
            ...
            <div className="mt-12 space-y-12">
                ...
                {/* Ingredients - Refined Theme (Fix Mismatch) */}
                <div className="bg-gray-50 dark:bg-gray-900 rounded-[2.5rem] p-10 md:p-12 border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden transition-colors">
                    <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5 dark:invert"></div>
                    <div className="relative z-10 text-center space-y-6">
                        ...
                        <div className="flex flex-wrap justify-center gap-2 max-w-4xl mx-auto">
                            {PRODUCT_DETAILS_CONTENT.ingredients.map((ing, idx) => (
                                <span key={idx} className="px-4 py-1.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:border-yellow-500 hover:text-yellow-600 transition-all cursor-default rounded-lg border border-gray-100 dark:border-gray-700 font-semibold text-xs transition-colors">
                                    {ing}
                                </span>
                            ))}
                        </div>
